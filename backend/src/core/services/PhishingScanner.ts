...
export class PhishingScanner {
  ...
  public async scan(url: string, content?: string, frontendAiScore?: number, frontendAiExplanation?: string): Promise<IScanResult> {
    ...
    const isEmailPlaceholder = url === 'http://email-analysis.local';
    ...
    if (!isEmailPlaceholder) {
      const [reputationRes, vtRes] = await Promise.all([
        this.reputationService.checkSafeBrowsing(url),
        this.virusTotalService.checkUrl(url),
      ]);
      reputationScore = reputationRes.score; // 0 or 100
      vtResult = vtRes;
    ...
    if (isEmailPlaceholder) {
      // If purely an email text analysis, the AI Score is the sole determinant
      totalScore = aiScore;
    } else {
